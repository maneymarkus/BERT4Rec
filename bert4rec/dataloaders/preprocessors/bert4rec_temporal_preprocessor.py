...
class BERT4RecTemporalPreprocessor(BasePreprocessor):
    ...
    def process_element(cls, sequence, timestamps) -> dict:
        ...
        :param timestamps: The corresponding timestamps to each element of the sequence.
        """
        # initiate return dictionary
        processed_features = dict()

        timestamps = tf.cast(timestamps, dtype=tf.int64)
    ...
    def prepare_inference(cls, data) -> dict:
        ...
        :param data: A tuple with a sequence and corresponding timestamps. The timestamps have to
            be given in seconds (as an integer) and should correspond to/be compatible with the
            timestamps created by the python time.time() method.
            (See: https://docs.python.org/3/library/time.html#time.time)
        :return:
        """
        sequence, timestamps = data
        ...
        # "predict the future"/generate the recommendation
        sequence = sequence[-cls.max_seq_len + 1:]
        timestamps = timestamps[-cls.max_seq_len + 1:]
        ...
        # add current timestamp at the end of the timestamps list as the recommendation should be
        # made "now"
    ...
    def _ds_map_fn(cls, sequence, timestamps):
        ...
        processed_features["input_timestamps"] = output[3]
